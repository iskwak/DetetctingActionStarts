...
def filter_video(opts, h5exp):
    ...
    if labels.shape[0] < 1500:
        ...
        print("%s: %d" % (os.path.basename(h5exp.filename), labels.shape[0]))
        filter += "short"
...
def get_exp_data(exp_file):
    ...
    # the video
    last_frame = last_frame + 50 + rng.randint(0, 50)
    org_labels = org_labels[:last_frame, :]

    # crop the features... consider any key thats not "labels" as a feature
    # matrix
    keys = list(exp_file.keys())
    keys = [key for key in keys if key != "labels"]
    feature_dict = {}
    for key in keys:
        temp = exp_file[key].value
        # feature_dict[key] = temp[:last_frame, :]
        temp = temp[:last_frame, :]
        # reshape temp into [sequence length by batch by features]
        feature_dict[key] = temp.reshape((temp.shape[0], 1, temp.shape[1]))
    ...
    # return org_features, pos_features, exp_name, mouse, date, labels,\
    #     org_labels
    return feature_dict, exp_name, mouse, date, labels, org_labels
...
def preprocess_features(opts, log, skip_log, out_data, exp_dir):
    ...
    for i in range(num_exps):
        ...
        with h5py.File(os.path.join(base_exp_dir, exp), "r") as exp_file:
            ...
            feature_dict, exp_name, mouse, date, labels, org_labels =\
                get_exp_data(exp_file)
            # org_features, pos_features, exp_name, mouse, date,\
            #     labels, org_labels = get_exp_data(exp_file)
            ...
            with h5py.File(os.path.join(exp_dir, exp), "w") as out_exp:
                ...
                for key in keys:
                    out_exp[key] = feature_dict[key].astype("float32")
