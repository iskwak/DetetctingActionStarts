...
def copy_movie_graphs(opts, all_exp_dir, h5_data, label_names, fps=30):
    ...
    for i in range(len(exp_names)):
        ...
        paths.create_dir(base_out)
        video_name = os.path.basename(exp["video_name"].value.decode("utf-8"))
        ...
        movie_file = os.path.join(
            opts["flags"].display_dir, exp["video_name"].value.decode("utf-8"))
        ...
        # for each label_name
        for label_name in label_names:
            csv_name = "predict_" + label_name.decode() + ".csv"
            create_html_file(
