...
gflags.DEFINE_boolean("cached", False, "Use cached?.")
...
class HDF5Sampler(object):
    ...
    def _batch_sampler(self):
        ...
        features = [
            torch.tensor(feature, requires_grad=True) for feature in features
        ]
        labels = torch.tensor(labels, requires_grad=False)
        masks = torch.tensor(masks, requires_grad=False)
        proc_labels = torch.tensor(proc_labels, requires_grad=False)
...
class HDF5BinnedSampler(HDF5Sampler):
    def __init__(self, rng, hdf5_data, mini_batch, feat_keys,
                 max_workers=2, seq_len=-1, max_queue=5,
                 use_pool=False, gpu_id=-1, feat_pre=None, label_pre=None,
                 use_cached=False, label_key="labels"):
        self.rng = rng
        self.data = hdf5_data
        self.use_pool = use_pool
        self.use_gpu = gpu_id
        ...
        self.label_dims =\
            self.data["exps"][self.exp_names[0]][self.label_key].shape[1]
        ...
        self.num_batch = int(numpy.ceil(self.num_exp / self.mini_batch))
        self.batch_idx = queue.Queue(self.num_batch)
        # next setup the binned sampling.
        self.create_video_bins()
        self.cache_data()
    ...
    def create_video_bins(self):
        lengths = []
        for exp_name in self.exp_names:
            lengths.append(
                self.data["exps"][exp_name][self.label_key].shape[0]
            )
        sort_idx = numpy.flip(numpy.argsort(lengths))
        lengths = numpy.asarray(lengths)[sort_idx]
        # group the videos by "bins"
        self.exp_names = self.exp_names[sort_idx]

        # a few options can be done here.
        # if the videos are being sampled one at a time with seq_len -1, then
        # then sorting the videos with length in mind doesn't make sense.
        # If the videos are being sampled in a mini batch. we can group the
        # videos into num_videos / mini_batch_size.
        # lets just group into batch size.
        num_groups = int(numpy.ceil(
            len(self.exp_names) / self.mini_batch
        ))
        idx = 0
        self.binned_batches = []
        num_iters = 0
        for i in range(num_groups):
            start_idx = i * self.mini_batch
            end_idx = numpy.min(
                [(i + 1)  * self.mini_batch, len(self.exp_names)]
            )
            self.binned_batches.append(
                list(range(start_idx, end_idx))
            )
            # iterations are the number of 64 frame chunks of the longest
            # video
            num_iters += int(numpy.ceil(lengths[self.binned_batches[-1]][0] / 64))
        self.iter_per_epoch = num_iters

    def reset(self):
        """Reset the sampler."""
        self.exp_idx = numpy.asarray(range(len(self.binned_batches)))
        if self.rng is not None:
            self.rng.shuffle(self.exp_idx)
        for i in range(self.num_batch):
            self.batch_idx.put(i)
    ...
    def _batch_sampler(self):
        ...
        # print "worker %d: loaded data, waiting to put..." % self.threadid
        # now actually get the batch
        idx_range = self.binned_batches[
            self.exp_idx[batch_id]
        ]

        # get the exps
        # sampled_idx = self.exp_idx[idx_range]
        # print(idx_range)
        sampled_exps = self.exp_names[idx_range]
    ...
    def _get_features(self, sampled_exps):
        ...
        max_seq_len = -1
        if self.seq_len < 1:
            # if the seq_len is less than one, then we want to use the full
            # sequence size.
            # loop over the
            for i in range(len(sampled_exps)):
                exp_name = sampled_exps[i]
                curr_label = self.data["exps"][exp_name][self.label_key][()]
                if max_seq_len < curr_label.shape[0]:
                    max_seq_len = curr_label.shape[0]
        else:
            max_seq_len = self.seq_len

        all_feats = [
            numpy.zeros(
                (max_seq_len, self.mini_batch, feat_dim), dtype="float32")
            for feat_dim in self.feat_dims
        ]
        all_labels = numpy.zeros(
            (max_seq_len, self.mini_batch, self.label_dims),
            dtype="float32")
        all_proc_labels = numpy.zeros(
            (max_seq_len, self.mini_batch, self.label_dims),
            dtype="float32")
        ...
        for i in range(len(sampled_exps)):
            # len(sampled_exps) should be the same as self.mini_batch
            exp_name = sampled_exps[i]
            curr_label = self.get_labels(exp_name)
            seq_len = numpy.min([curr_label.shape[0], max_seq_len])

            all_labels[:seq_len, i, :] = curr_label[:seq_len, :]
            all_masks[:seq_len, i, :] = 1

            feat = self.get_features(exp_name)
            for j in range(len(feat)):
                if self.feat_pre is not None:
                    feat[j][:seq_len, :] = self.feat_pre(
                        feat[j][:seq_len, :], self.feat_keys[j])
                all_feats[j][:seq_len, i, :] = feat[j][:seq_len, :]

            if self.label_pre is not None:
                all_proc_labels[:seq_len, i, :] =\
                    self.label_pre(all_labels[:seq_len, i, :])
            else:
                all_proc_labels[:seq_len, i, :] = all_labels[:seq_len, i, :]

        return all_feats, all_labels, all_proc_labels, all_masks, sampled_exps

    def get_labels(self, exp_name):
        if self.use_cached:
            curr_label = self.all_labels[exp_name]
        else:
            curr_label = self.data["exps"][exp_name][self.label_key][()]
        return curr_label

    def get_features(self, exp_name):
        if self.use_cached:
            features = self.all_feats[exp_name]
        else:
            features = []
            for i in range(len(self.feat_keys)):
                feat = self._get_field(exp_name,
                                       self.feat_keys[i])
    ...
    def cache_data(self):
        if self.use_cached == True:
            ...
            for i in range(self.num_exp):
                ...
                curr_label = self.data["exps"][exp_name][self.label_key][()]
                seq_len = curr_label.shape[0]

                # get features
                self.all_feats[self.exp_names[i]] = []
                for j in range(len(self.feat_dims)):
                    self.all_feats[exp_name].append(
                        self._get_field(exp_name, self.feat_keys[j])
                    )

                self.all_labels[exp_name] = curr_label
                # self.all_masks[exp_name] = numpy.ones(
                #     (seq_len, self.label_dims), dtype="uint8"
                # )
...
class HDF5CachedSampler(object):
    """HDF5CachedSampler

    Samples HDF5 files. Retrives the provided keys. Caches all the data first.
    """
    def __init__(self, rng, hdf5_data, mini_batch, feat_keys,
                 max_workers=2, seq_len=-1, max_queue=5,
                 use_pool=False, gpu_id=-1, feat_pre=None, label_pre=None,
    ...
    def cache_data(self):
        self.all_feats = [
            numpy.zeros(
                (self.seq_len, self.num_exp, feat_dim), dtype="float32")
            for feat_dim in self.feat_dims
        ]
        self.all_labels = numpy.zeros(
            (self.seq_len, self.num_exp, self.label_dims),
            dtype="uint8")
        # all_proc_labels = numpy.zeros(
        #     (self.seq_len, self.mini_batch, self.label_dims),
        #     dtype="float32")
        self.all_masks = numpy.zeros(
            (self.seq_len, self.num_exp, self.label_dims),
            dtype="uint8")
        ...
        for i in range(self.num_exp):
            ...
            self.all_labels[:seq_len, i, :] = curr_label[:seq_len, :]
            self.all_masks[:seq_len, i, :] = 1
            ...
            for j in range(len(self.feat_keys)):
                ...
                #     feat[:seq_len, :] = self.feat_pre(
                #         feat[:seq_len, :], self.feat_keys[j])
                self.all_feats[j][:seq_len, i, :] = feat[:seq_len, :]
    ...
    def _get_features(self, sampled_exps):
        ...
        for i in range(len(sampled_exps)):
            # len(sampled_exps) should be the same as self.mini_batch
            exp_name = sampled_exps[i]
            exp_idx = numpy.argwhere(exp_name == self.exp_names)[0][0]

            # curr_label = self.data["exps"][exp_name]["labels"][()]
            curr_label = self.all_labels[:, exp_idx, :]
            # seq_len = numpy.min([curr_label.shape[0], self.seq_len])
            seq_len = self.seq_len

            all_labels[:seq_len, i, :] = curr_label[:seq_len, :]
            all_masks[:seq_len, i, :] = self.all_masks[:seq_len, exp_idx, :]
            # all_masks[:seq_len, i, :] = 1

            for j in range(len(self.feat_keys)):
                feat = self.all_feats[j][:, exp_idx, :]
                all_feats[j][:seq_len, i, :] = feat[:seq_len, :]
                # feat = self._get_field(sampled_exps[i],
                #                        self.feat_keys[j])
                if self.feat_pre is not None:
...
class HDF5CachedFrameSampler(object):
    """HDF5CachedFrameSampler

    Samples random feature frames from HDF5 files.

    If use_pool is true, the sampler will be multithreaded and maybe faster.
    Unfortunately there's no guarentee that the pooled data loader is faster.

    Caches the data as well.
    """
    def __init__(self, rng, hdf_data, mini_batch, feat_keys, max_workers=2,
                 frames=[0], max_queue=5, use_pool=False, gpu_id=-1,
                 feat_pre=None, label_pre=None):
        self.rng = rng
        self.data = hdf_data
        self.use_pool = use_pool
        self.use_gpu = gpu_id
        ...
        self.neg_idx = []
        # Find the positive examples (and figure out the negative bouts).
        self.seq_len = 1500 # does this make sense?
    ...
    def _find_positives(self):
        ...
        for i in range(self.num_exp):
            ...
            exp = self.data["exps"][exp_name]
            # temp = numpy.argwhere(exp["labels"][()] > 0)
            temp = numpy.argwhere(exp["labels"][:self.seq_len, :] > 0)
            seq_len = exp["labels"].shape[0]
            seq_len = numpy.min([seq_len, self.seq_len])
            # print("\t%d" % seq_len)
    ...
    def get_batch(self, batch_size, idx, idx_range):
        ...
        all_feats = [
            numpy.zeros(
                (batch_size, feat_dim), dtype="float32")
            for feat_dim in self.feat_dims
        ]

        labels = torch.zeros(batch_size, self.label_dims)
        for sample_i in range(batch_size):
            movie_id = idx[idx_range[sample_i]][0]
            frame_id = idx[idx_range[sample_i]][1]
            for j in range(len(self.feat_keys)):
                all_feats[j][sample_i, :] =\
                    self.all_feats[j][frame_id, movie_id, :]

            label = self.all_labels[frame_id, movie_id]
            labels[sample_i, :] = torch.from_numpy(label)

        # concatenate the labels?
        all_feats = numpy.concatenate(all_feats, axis=1)
    ...
    def batch_sampler(self):
        ...
        neg_features = torch.tensor(neg_features)
        features = [
            torch.cat([pos_features, neg_features], dim=0)
        ]
        labels = torch.cat([pos_labels, neg_labels])
