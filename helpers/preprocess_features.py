...
def get_exp_data(ipca, exp_file):
    ...
    org_labels = org_labels[:1500, :]
    smooth_data(opts, org_labels)

    org_features = exp_file["hoghof"].value
    org_features = org_features[:1500, :]

    reduced = numpy.dot(org_features - ipca.mean_,
                        ipca.components_[:200, :].T)
    ...
    return reduced, exp_name, mouse, date, labels


def preprocess_features(opts, log, skip_log, out_data):
    ...
    # lstm.
    all_feat = numpy.concatenate(all_feat, axis=1)
    all_feat = all_feat.astype("float32")
    all_labels = numpy.concatenate(all_labels, axis=1)
