...
def _init_network(opts, h5_data, label_weight):
    ...
    if opts["flags"].arch == "concat":
        ...
        network = hantman_hungarian.HantmanHungarianImageConcat(
...
def _get_seq_mini_batch(opts, batch_id, h5_data, idx):
    ...
    all_feats = [
        # torch.zeros(opts["flags"].seq_len, batch_size, 224*224*3)
        torch.zeros(opts["flags"].seq_len, batch_size, 224*224)
        for i in range(2)
    ]

    base_path = '/media/drive1/data/hantman_frames/'
    for i in range(batch_size):
        temp_idx = idx[vid_idx[i]]
        # print(temp_idx)
        vid_path = os.path.join(base_path, temp_idx.decode('UTF-8'), 'frames')
        num_frames = h5_data["exps"][temp_idx]['org_labels'].shape[0]
        seq_len = min(num_frames, opts["flags"].seq_len)
        # take only seq length frames
        # tic = time.time()
        # cow = torch.zeros(opts["flags"].seq_len, batch_size, 224*224*3).cuda()
        # moo = torch.zeros(opts["flags"].seq_len, batch_size, 224*224*3).cuda()
        for j in range(seq_len):
            img_name = os.path.join(vid_path, "%05d.jpg" % j)
            img = Image.open(img_name)
            img_side = preproc(img.crop((0, 0, 352, 260)))
            img_front = preproc(img.crop((352, 0, 704, 260)))
            all_feats[0][j, i, :] = img_side[0, :].view(-1)
            all_feats[1][j, i, :] = img_front[0, :].view(-1)
