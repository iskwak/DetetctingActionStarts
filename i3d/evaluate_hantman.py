...
def write_features(out_dir, queue, feat_key):
  ...
  while True:
    exp_name, front_fc, side_fc = queue.get()
    # print(exp_name)
    if exp_name == "done":
    ...
    with h5py.File(feature_file, "w") as out_data:
      ...
      out_data[front_key] = front_fc
      side_key = "%s_side" % feat_key
      out_data[side_key] = side_fc
  ...
  with tf.Session() as sess:
    ...
    with open(filelist, "r") as fid:
      ...
      while exp_name:
        ...
        print(filename)
        front_fc = process_video(
          sess, rgb_endpoints, rgb_input,
          batch_size * gpu_num, window_start, window_size,
          filename, eval_type)

        filename = os.path.join(movie_dir, "side", "%s.avi" % exp_name)
        print(filename)
        side_fc = process_video(
          sess, rgb_endpoints, rgb_input,
          batch_size * gpu_num, window_start, window_size,
          filename, eval_type)
        print("\tfinished feature computation: %f" % (time.time() - proc_tic))
        # write_tic = time.time()
        write_queue.put((exp_name, front_fc, side_fc))
