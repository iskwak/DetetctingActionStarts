...
_CHECKPOINT_PATHS = {
    ...
    # 'rgb': '/nrs/branson/kwaki/outputs/i3d_ff/M134/front/networks/epoch_0040.ckpt',
    ...
    # 'rgb_imagenet': '/nrs/branson/kwaki/outputs/i3d_ff/M134/front/networks/epoch_0040.ckpt',
...
def get_video_frames(batch_size, cap, num_frames, window_size=16, window_start=-8,
  ...
  # print("\t\tcreating inital batch")
  prev_views1, prev_views2, frame_end = create_initial_batch(
    cap, crop_size, window_size, window_start)
  # print("\t\tfinished initial batch setup")
  # print("\t\tSetting frame to 0")
  # cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
  # print("\t\tdone setting frame to 0")
  ...
  while frame_num < num_frames:
    ...
    for i in range(valid_len):
      ...
      batch_view1[i, :, :, :] = np.asarray(prev_views1)
      batch_view2[i, :, :, :] = np.asarray(prev_views2)
      # view1_data.append(prev_views1)
      # view2_data.append(prev_views2)
      prev_views1.pop(0)
      prev_views2.pop(0)
      frame_num += 1
      if frame_end < num_frames:
    ...
    queue.put((valid_len, batch_view1, batch_view2, frame_num, frame_end))
...
def _get_label_weight(opts, data, label_names):
  """Get number of positive examples for each label."""
  # tic = time.time()
  # experiments = data["exp_names"][()]
  # label_mat = np.zeros((experiments.size, len(label_names) + 1))
...
def main(unused_argv):
  ...
  with h5py.File(opts["flags"].train_file, "r") as train_data:
    ...
    label_weight = _get_label_weight(opts, train_data, g_label_names)
    ...
    if eval_type in ['rgb', 'rgb600', 'joint']:
      ...
      split_num_frames = int(window_size / 2)
      rgb_input = (
        tf.placeholder(tf.float32,
          shape=(batch_size, split_num_frames, _IMAGE_SIZE, _IMAGE_SIZE, 3)),
        tf.placeholder(
          tf.float32,
          shape=(batch_size, split_num_frames, _IMAGE_SIZE, _IMAGE_SIZE, 3))
        )
      ...
      new_labels = tf.placeholder(tf.float32, shape=(batch_size, len(g_label_names) + 1))
      ...
      with tf.device('/gpu:0'):
        ...
        avg = rgb_endpoints['averaged']
        avg_next = rgb_endpoints['averaged_next']
        # test_var = rgb_endpoints["test"]
        new_cost, optimizer, class_cost, sim_cost = create_criterion(
          opts, rgb_logit, new_labels, avg, avg_next, weight=label_weight,
          name="hantman_crit")
      rgb_endpoints = [
        rgb_logit,
        new_cost,
        ...
        class_cost,
        sim_cost,
        # tf.nn.softmax(rgb_logit, axis=1)
      ]
      ...
      for variable in tf.global_variables():
        if variable.name.split('/')[0] == 'RGB':
          if 'hantman' not in variable.name:
            temp = variable.name.replace(':0', '')
            temp = temp.replace("/base", "")
            rgb_variable_map[temp] = variable
    ...
    with tf.Session() as sess:
      ...
      rgb_saver = tf.train.Saver(var_list=rgb_variable_map, reshape=True)
      # hack!!!
      # network_dir = os.path.join(opts["flags"].out_dir, "networks", "epoch_0030.ckpt")
      # network_dir = network_dir.replace("odas_unfrozen2", "odas_unfrozen")
      # # network_dir = network_dir.replace("odas2", "odas")
      # rgb_saver.restore(sess, network_dir)
      train_network(opts, sess, rgb_saver, sampler, rgb_endpoints,
                    rgb_input, new_labels)
      print("hi")


def create_criterion(opts, endpoint, labels, avg, avg_next, weight=None, name=None):
  ...
  if weight is not None:
    ...
    class_cost = label_weights * cost

  sim_cost = tf.reduce_sum((avg - avg_next) * (avg - avg_next), axis=1)
  cost = tf.reduce_mean(class_cost + 0.01 * sim_cost)

  last_layers = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "RGB/inception_i3d/Logits/")
  optim = tf.train.AdamOptimizer(opts["flags"].learning_rate, name=name)\
   .minimize(cost, var_list=last_layers)
  # optim = tf.train.AdamOptimizer(opts["flags"].learning_rate, name=name)\
  #  .minimize(cost)

  return cost, optim, class_cost, sim_cost
...
def train_network(opts, sess, saver, sampler, rgb_endpoints, rgb_input, new_label):
  ...
  count2_fid = open(
    os.path.join(opts["flags"].out_dir, "counts2.txt"), "w")

  # temp_fid = open(
  #   os.path.join(opts["flags"].out_dir, "debug.txt"), "w")

  # temp_fid.write("num batch,%d\n" % sampler.num_batch)

  half_batch = int(opts["flags"].hantman_mini_batch / 2)

  print("num iter per epoch: %d" % sampler.num_batch)
  for i in range(opts["flags"].total_epochs):
    ...
    for j in range(sampler.num_batch):
      ...
      window_start = batch[0][:, :64, :, :, :]
      followup = batch[0][:, 64:, :, :, :]

      feed_dict[rgb_input] = (window_start, followup)
      ...
      # print(out[-1])
      # print(out[1])
      # print("\tdata: %f" % data_times[-1])
      # print("\tnetwork: %f" % network_times[-1])
      # temp_fid.write("network,%f\n" % network_times[-1])
      # temp_fid.flush()
      count += sum(predict == gt)
      count2 += sum(predict[:half_batch] == gt[:half_batch])
      # print(predict[:half_batch] == gt[:half_batch])
      # print(predict == gt)
    ...
    count2_fid.write(
      "%f\n" %
      (count2 / (sampler.num_batch * opts["flags"].hantman_mini_batch))
    )
    count_fid.flush()
    count2_fid.flush()
    # import pdb; pdb.set_trace()
