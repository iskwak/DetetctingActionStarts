...
python -m i3d.eval_finetune --filelist ${LISTDIR}debug_list.txt --gpus 1 --batch_size 6 --window_size 64 --window_start -31 --movie_dir "/nrs/branson/kwaki/data/videos/hantman_rgb/" --out_dir ${OUTDIR} --feat_key ${BASEKEY} --eval_type rgb --logfile ${LOGOUT}$1_debug.txt --model ${MODEL} --type side --feat_dir ${FEATDIR}
