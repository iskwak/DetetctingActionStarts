...
tf.flags.DEFINE_string('eval_type', 'rgb', 'rgb, flow')
...
tf.flags.DEFINE_string('model', '', 'model file.')
tf.flags.DEFINE_string('feature_dir', '', 'feature folder.')
...
def write_features(out_dir, queue, feat_key):
  ...
  while True:
    exp_name, frame_fc, frame_logits = queue.get()
    # print(exp_name)
    if exp_name == "done":
    ...
    with h5py.File(feature_file, "w") as out_data:
      out_data[feat_key] = frame_fc
      out_data['logits'] = frame_logits
...
def main(unused_argv):
  ...
  # for this version, save the HDF's into features/<feat_key>
  # base_feat_dir = os.path.join(out_dir, 'features')
  # if not os.path.exists(base_feat_dir):
  #   os.mkdir(base_feat_dir)
  # feat_dir = os.path.join(base_feat_dir, feat_key)
  feat_dir = FLAGS.feature_dir
  ...
  if eval_type in 'rgb':
    ...
    rgb_endpoints = rgb_endpoints['averaged']
  ...
  if eval_type in 'flow':
    ...
    # rgb_endpoints = rgb_vars
    # rgb_endpoints = tf.concat(rgb_vars, 0)
    # rgb_endpoints = [rgb_endpoints['averaged'],rgb_endpoints['logit2']]
    rgb_endpoints = rgb_endpoints['averaged']
  ...
  with tf.Session() as sess:
    ...
    if eval_type in 'rgb':
      ...
      # rgb_saver.restore(sess, model)
      rgb_saver.restore(sess, modelfile)
    ...
    logname = os.path.join(out_dir, 'files_proc.txt')
    log_fid = open(logname, "w")
    timingname = os.path.join(out_dir, 'timing.csv')
    timing_fid = open(timingname, "w")
    with open(filelist, "r") as fid:
      exp_name = fid.readline().strip()
      # filename = os.path.join(movie_dir, exp_name, "movie_comb.avi")
      while exp_name:
        feature_file = os.path.join(feat_dir, exp_name)
        ...
        # print(filename)
        frame_fc, frame_logits = process_video(
        ...
        timing_fid.write("processed,%f\n" % (time.time() - proc_tic))
        timing_fid.flush()
        # write_tic = time.time()
        write_queue.put((exp_name, frame_fc, frame_logits))
...
def process_video(sess, rgb_endpoints, rgb_input, batch_size,
  ...
  frame_logits = np.zeros((num_frames, 7))

  # print("proc'ing first batch")
  data = queue.get()
  frame_proc = sess.run(rgb_endpoints, {rgb_input:data[1]})
