...
import threading
import i3d.odas_i3d_eval as i3d
import cv2
import time
import os
import h5py
...
def create_initial_batch(cap, crop_size, window_size, window_start):
  ...
  # want to add frame 0 as many times as the window start is 0 or less.
  if -window_start > window_size:
    # if -window_start > window_size, then all the frames are going to be
    # negative, and non positive. so just set to window size.
    negative_frames = window_size
  else:
    # otherwise, just use -window_start frames.
    negative_frames = np.max(-window_start, 0)
  prev_frames = [frame for i in range(negative_frames)]

  # -1 so that the main loop can add the last frame of this example to the
  # batch.
  window_end = window_size + window_start - 1
  # next need the frame start, just in case window_start is > 0 then the
  # starting frame might not be zero.
  frame_start = np.max([0, window_start])
  cap.set(cv2.CAP_PROP_POS_FRAMES, frame_start)

  # window_end = window_size + window_start - 2
  frame_num = frame_start
  for i in range(frame_start, window_end):
    retval, frame = cap.read()
