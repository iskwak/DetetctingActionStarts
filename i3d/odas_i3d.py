...
class InceptionI3d(snt.AbstractModule):
  ...
  def _build(self, inputs, is_training, dropout_keep_prob=1.0):
    ...
    with tf.variable_scope("base", reuse=True):
      net2 = Unit3D(output_channels=64, kernel_shape=[7, 7, 7],
                  stride=[2, 2, 2], name=end_point)(net2, is_training=is_training)

      end_point = 'MaxPool3d_2a_3x3'
      net2 = tf.nn.max_pool3d(net2, ksize=[1, 1, 3, 3, 1], strides=[1, 1, 2, 2, 1],
                            padding=snt.SAME, name=end_point)

      end_point = 'Conv3d_2b_1x1'
      net2 = Unit3D(output_channels=64, kernel_shape=[1, 1, 1],
                  name=end_point)(net2, is_training=is_training)

      end_point = 'Conv3d_2c_3x3'
      net2 = Unit3D(output_channels=192, kernel_shape=[3, 3, 3],
                  name=end_point)(net2, is_training=is_training)
    ...
    end_point = 'Mixed_3b'
    with tf.variable_scope(end_point, reuse=True):
      with tf.variable_scope('Branch_0', reuse=True):
        branch_0 = Unit3D(output_channels=64, kernel_shape=[1, 1, 1],
                          name='Conv3d_0a_1x1')(net2, is_training=is_training)
      with tf.variable_scope('Branch_1', reuse=True):
        branch_1 = Unit3D(output_channels=96, kernel_shape=[1, 1, 1],
                          name='Conv3d_0a_1x1')(net2, is_training=is_training)
        branch_1 = Unit3D(output_channels=128, kernel_shape=[3, 3, 3],
                          name='Conv3d_0b_3x3')(branch_1,
                                                is_training=is_training)
      with tf.variable_scope('Branch_2', reuse=True):
        branch_2 = Unit3D(output_channels=16, kernel_shape=[1, 1, 1],
                          name='Conv3d_0a_1x1')(net2, is_training=is_training)
        branch_2 = Unit3D(output_channels=32, kernel_shape=[3, 3, 3],
                          name='Conv3d_0b_3x3')(branch_2,
                                                is_training=is_training)
      with tf.variable_scope('Branch_3', reuse=True):
        branch_3 = tf.nn.max_pool3d(net2, ksize=[1, 3, 3, 3, 1],
                                    strides=[1, 1, 1, 1, 1], padding=snt.SAME,
                                    name='MaxPool3d_0a_3x3')
        branch_3 = Unit3D(output_channels=32, kernel_shape=[1, 1, 1],
                          name='Conv3d_0b_1x1')(branch_3,
      ...
      net2 = tf.concat([branch_0, branch_1, branch_2, branch_3], 4)
    ...
    net = tf.nn.max_pool3d(net, ksize=[1, 3, 3, 3, 1], strides=[1, 2, 2, 2, 1],
                           padding=snt.SAME, name=end_point)
    end_points[end_point] = net
    net2 = tf.nn.max_pool3d(net2, ksize=[1, 3, 3, 3, 1], strides=[1, 2, 2, 2, 1],
                           padding=snt.SAME, name=end_point)
    if self._final_endpoint == end_point: return net, end_points
    ...
    net = tf.nn.max_pool3d(net, ksize=[1, 2, 2, 2, 1], strides=[1, 2, 2, 2, 1],
                           padding=snt.SAME, name=end_point)
    net2 = tf.nn.max_pool3d(net2, ksize=[1, 2, 2, 2, 1], strides=[1, 2, 2, 2, 1],
                           padding=snt.SAME, name=end_point)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points
      ...
      with tf.variable_scope('Branch_3'):
        ...
        branch_3 = Unit3D(output_channels=128, kernel_shape=[1, 1, 1],
                          name='Conv3d_0b_1x1')(branch_3,
                                                is_training=is_training)
      net = tf.concat([branch_0, branch_1, branch_2, branch_3], 4)
      # flattened = tf.layers.Flatten()(net)
    end_points[end_point] = net
    # end_points["flattened"] = flattened
    if self._final_endpoint == end_point: return net, end_points
    ...
    with tf.variable_scope(end_point):
      ...
      end_points['averaged'] = tf.layers.Flatten()(net)
      logits2 = Unit3D(output_channels=7,
                      kernel_shape=[1, 1, 1],
                      activation_fn=None,
                      use_batch_norm=False,
                      use_bias=True,
                      name='hantman_Conv3d_0c_1x1')(logits, is_training=is_training)

      if self._spatial_squeeze:
        logits = tf.squeeze(logits, [2, 3], name='SpatialSqueeze')
        logits2 = tf.squeeze(logits2, [2, 3], name="hantman_Squeeze")
    averaged_logits = tf.reduce_mean(logits, axis=1)
    averaged_logits2 = tf.reduce_mean(logits2, axis=1)
    end_points[end_point] = averaged_logits
    end_points["logit2"] = averaged_logits2
    end_point = 'Logits'

    with tf.variable_scope(end_point, reuse=True):
      net2 = tf.nn.avg_pool3d(net2, ksize=[1, 2, 7, 7, 1],
                             strides=[1, 1, 1, 1, 1], padding=snt.VALID)
      end_points['preflat_next'] = net2

      net2 = tf.nn.dropout(net2, dropout_keep_prob)
      logits = Unit3D(output_channels=self._num_classes,
                      kernel_shape=[1, 1, 1],
                      activation_fn=None,
                      use_batch_norm=False,
      ...
      end_points['averaged_next'] = tf.layers.Flatten()(net2)
      if self._spatial_squeeze:
        logits = tf.squeeze(logits, [2, 3], name='SpatialSqueeze')
    averaged_logits_smooth = tf.reduce_mean(logits, axis=1)
    end_points[end_point + "_smooth"] = averaged_logits_smooth

    end_points["test"] = tf.global_variables()[-4]
