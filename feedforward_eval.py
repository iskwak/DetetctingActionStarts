...
gflags.DEFINE_string("feature_dir", None, "Directory to save features.")
...
def _init_model(opts, label_weight):
    ...
    if opts["flags"].load_network is not None:
        model_dict = torch.load(opts["flags"].load_network)
...
def _process_seqs(opts, step, network, sampler, criterion, name):
    ...
    for i in range(sampler.num_batch):
        ...
        for j in range(0, num_chunks):
            ...
            # print("\t%d" % idx2)
            chunk = [
                Variable(inputs[0][idx1:idx2, :1, :, :], requires_grad=False).cuda(),
                Variable(inputs[1][idx1:idx2, :1, :, :], requires_grad=False).cuda(),
...
def main(argv):
    ...
    if not os.path.exists(opts["flags"].feature_dir):
        ...
        exp_dir = os.path.join(
            os.path.dirname(opts["flags"].train_file),
            "exps",
