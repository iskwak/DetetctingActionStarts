...
requirejs(["d3", "d3-format"],
    function(d3, d3_format) {
        ...
        d3.csv("loss.csv", type, function(error, data) {
            ...
            //}));
            y.domain([
              d3.min(scores, function(c) { return d3.min(c.values, function(d) { return d.value; }); }),
              d3.max(scores, function(c) { return d3.max(c.values, function(d) { return d.value; }); })
            ]);
            ...
            function create_drop_line(scores, idx, x0, offset) {
                ...
                drop_lines[idx].attr('transform', `translate(${x(d.iteration)}, ${y(d.value)})`);
                drop_lines[idx].select('line.x')
                    .attr('x1', 0)
                    .attr('x2', -x(d.iteration))
                    .attr('y1', 0)
                    .attr('y2', 0);
                ...
                divs[idx].transition()
                    .duration(200)
                    .style("opacity", .9);
                divs[idx].html(d.iteration + "<br/>" + d.value)
