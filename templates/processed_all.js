define(["d3", "d3-format"], function(d3, d3_format) {
    return {
        greet: function(csv_filename) {
            ...
            d3.csv("processed_lift.csv", type, function(error, data) {
                ...
                var liftscores = data.columns.slice(1).map(function(id) {
                    console.log(id);
                    if(id == "ground truth") {
                        dict = {
                            id: "lift ground truth",
                ...
                // hack...
                liftscores = [liftscores[0], liftscores[1]];
                d3.csv("processed_hand.csv", type, function(error, data) {

                var handscores = data.columns.slice(1).map(function(id) {
                    console.log(id);
                    if(id == "ground truth") {
                        dict = {
                            id: "hand ground truth",
                            values: data.map(function(d) {
                                return {
                                    frame: d.frame,
                                    value: d[id],
                ...
                // hack...
                handscores = [handscores[0], handscores[1]];

                all_scores = liftscores.concat(handscores);
                d3.csv("processed_grab.csv", type, function(error, data) {
                ...
                all_scores = all_scores.concat(scores);
                d3.csv("processed_suppinate.csv", type, function(error, data) {
                var scores = data.columns.slice(1).map(function(id) {
                    console.log(id);
                    if(id == "ground truth") {
                        dict = {
                            id: "suppinate ground truth",
                            values: data.map(function(d) {
                                return {
                                    frame: d.frame,
                                    value: d[id],
                ...
                d3.csv("processed_mouth.csv", type, function(error, data) {
               var scores = data.columns.slice(1).map(function(id) {
                    console.log(id);
                    if(id == "ground truth") {
                        dict = {
                            id: "mouth ground truth",
                            values: data.map(function(d) {
                                return {
                                    frame: d.frame,
                                    value: d[id],
                ...
                d3.csv("processed_chew.csv", type, function(error, data) {
                var scores = data.columns.slice(1).map(function(id) {
                    console.log(id);
                    if(id == "ground truth") {
                        dict = {
                            id: "chew ground truth",
                            values: data.map(function(d) {
                                return {
                                    frame: d.frame,
                                    value: d[id],
                ...
                for(var i = 0; i < scores.length; i++) {
                    if(scores[i].id != "image" && scores[i].id != "nearest") {
                ...
                function mousemove() {
                    ...
                    for(var i = 0; i < scores.length; i++) {
                        create_drop_line(scores, i, x0, 0);
                    }
                ...
                function create_drop_line(scores, idx, x0, offset) {
                    ...
                    if(d.nearest == undefined) {
                        divs[idx].html(d.frame + "<br/>" + d.value + "<br/>" + d.id)
                        ...
                        divs[idx].html(d.frame + "<br/>" + d.value + "<br/>" + d.nearest + "<br/>" + d.id)
                            .style("left", (d3.event.pageX + 10) + "px")
                            .style("top", (d3.event.pageY - 28 - offset) + "px")
                            //.style("left", (x(d.frame) + 60) + "px")
                            //.style("top", (y(d.value) - 15 - offset) + "px")
                ...
                function add_draw_points(scores, idx, space, d_x, d_y) {
                    ...
                    var color = function(d) {
                        if(d.nearest != "N/A" && d.nearest != "no match" && d.nearest != undefined) {
                            // a "number"
                            return "green";
                        } else if(d.nearest == "no match") {
                            // bad peak
                            return "red";
                        } else {
                            return z(id);
            ...
            function type(d, _, columns) {
                ...
                for (var i = 0; i < columns.length; ++i) {
                    ...
                    if(col_name != "image" && col_name != "nearest") {
                        d[col_name] = +d[col_name];
                    }
                    // if(col_name == "image") {
                    //     d[col_name] = d[col_name]
                    // } else if(col_name == "nearest") {
                    //     // if(d[col_name] != "N/A" || d[col_name] != "no match") {
                    //     //     d[col_name] = +d[col_name]
                    //     // }
                    //     d[col_name] = d[col_name];
                    // } else {
