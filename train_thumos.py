...
gflags.DEFINE_string("label_key", None, "Label keys to use.")
...
def train_videos(opts, step, timings, network, blob, criterion, optimizer):
    """Train one batch of videos, updating the hidden state along the way."""
    ...
    chunk_size = 64
    idx = 0
    total_frames = inputs[0].shape[0]
    hidden = get_hidden(opts, network)
    while idx < total_frames:
        new_input = [
            feats[idx:idx+64, :, :].detach() for feats in inputs
        ]
        for feats in new_input:
            feats[idx:idx+64, :, :].requires_grad=True
        ...
        step += 1
        hidden = (update_hid[0].detach(), update_hid[1].detach())
        idx += 64
...
def train_lstm_epoch(opts, step, network, optimizer, criterion, sampler):
    ...
    for i in range(sampler.num_batch):
        ...
        step = train_videos(opts, step, timings, network, blob, criterion, optimizer)
