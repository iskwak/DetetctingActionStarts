...
def _predict_write(opts, out_dir, network, h5_data, feat_idx, vid_idx):
    ...
    inputs, org_labels, sample_idx = get_random_batch(opts, h5_data, vid_idx)

    predict = network["predict_batch"](*inputs)
    # ref_array, mask_array, false_neg, false_pos = create_match_array(
    #     predict, org_labels)

    # outputs = network["cost"](inputs[0], inputs[1],
    #                           ref_array, mask_array, false_neg)
    ...
    for i in range(len(sample_idx)):
        idx = sample_idx[i]
        vid = vid_idx[idx]
        labels.append(inputs[1][:, i, :].reshape(50, 1, 6))
...
def get_random_batch(opts, h5_data, idx):
    """Helper to get a random batch..."""
    # randomly sample a couple of videos.
    rng = np.random.RandomState(456)
    feat_dims = h5_data["exps"][idx[0]]["reduced"].value.shape[2]
    batch_size = 5
    sample_idx = rng.choice(len(idx), replace=False, size=batch_size)
    all_feats = np.zeros(
        (50, batch_size, feat_dims), dtype=theano.config.floatX)
    all_labels = np.zeros((50, batch_size, 6), dtype=theano.config.floatX)
    all_org_labels = np.zeros((50, batch_size, 6), dtype=theano.config.floatX)

    for i in range(batch_size):
        vid_idx = idx[sample_idx[i]]
        # vid_idx = idx[i]
        feat = h5_data["exps"][vid_idx]["reduced"].value
        labels = h5_data["exps"][vid_idx]["labels"].value
        org_labels = h5_data["exps"][vid_idx]["org_labels"].value

        # search for the labels
        label_idx = np.argwhere(org_labels)
        row_idx = np.argwhere(label_idx[:, 1] == (i % 3))[0][0]
        start_idx = label_idx[row_idx, 0] - 25
        end_idx = label_idx[row_idx, 0] + 25

        end_idx = min(end_idx, feat.shape[0])
        start_idx = end_idx - 50

        all_labels[:, i, :] = labels[start_idx:end_idx, 0, :]
        all_org_labels[:, i, :] = org_labels[start_idx:end_idx, :]
        all_feats[:, i, :] = feat[start_idx:end_idx, 0, :]

    return [all_feats, all_labels], all_org_labels, sample_idx
...
if __name__ == "__main__":
    ...
    # load the data
    with h5py.File(opts["flags"].filename, "a") as h5_data:
