...
g_out_dir = '/media/drive3/kwaki/data/jigsaw/20180529_jigsaw_base'
...
def create_clip(opts, video_file, cap, start_frame, end_frame, behav_id):
    ...
    cap.set(cv2.cv.CV_CAP_PROP_POS_FRAMES, start_idx)
...
def create_hdfs(opts, out_data, exp_dir):
    ...
    for i in range(len(label_files)):
        ...
        label_base = os.path.basename(os.path.splitext(label_files[i])[0])
        video_base = os.path.basename(os.path.splitext(video_files[i])[0])
        if label_base not in video_base:
            import pdb; pdb.set_trace()
            # search for label_base name?
        org_labels, labels, all_frames =\
            process_label_file(opts, label_files[i], video_files[i])
        ...
        with h5py.File(exp_hdf_name, "w") as exp_data:
            ...
            exp_data["video_name"] = video_files[i]
            exp_data.create_dataset(
                "frames", all_frames.shape, chunks=True, compression="gzip", dtype='uint8')
