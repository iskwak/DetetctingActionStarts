...
base_dir = '/localhome/kwaki/outputs/test_predict_comp3/';
...
for i = 1:length(postproc_exps),
    ...
    if strcmp(postproc_exps{i}, 'M134_20150505_v037')
        1;
    end
    ...
    for j = 1:length(postproc_fields),
        ...
        if ~isempty(postproc(i).(postproc_fields{j}))
            cvs_data(postproc(i).(postproc_fields{j})(1), 1) = 1;
        end
        % apply the gt data.
        if ~isempty(gtdata(gt_idx).(label_fields{j}))
            cvs_data(gtdata(gt_idx).(label_fields{j})(1), 2) = 1;
        end
