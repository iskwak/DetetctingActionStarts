...
def _log_outputs(opts, step, network, label_weight):
    ...
    train_dicts = post_processing.process_outputs(
        predict_dir, "", g_label_names, frame_thresh=frame_threshold)
...
def _get_seq_mini_batch(opts, batch_id, h5_data, idx):
    ...
    # all_img_side = np.zeros(
    #     (opts["flags"].seq_len, batch_size, feat_dims),
    #     dtype="float32")
    # all_img_front = np.zeros(
    #     (opts["flags"].seq_len, batch_size, feat_dims),
    #     dtype="float32")
    # all_img_side = torch.zeros(
    #     opts["flags"].seq_len, batch_size, feat_dims
    # )
    # all_img_front = torch.zeros(
    #     opts["flags"].seq_len, batch_size, feat_dims
    # )
    ...
    #     opts["flags"].seq_len, batch_size, 6
    # )
    # all_masks = torch.zeros(
    #     opts["flags"].seq_len, batch_size, 6
    # )
    # all_org_labels = torch.zeros(
    #     opts["flags"].seq_len, batch_size, 6
    # )
    ...
    for i in range(batch_size):
        ...
        # # temp_idx = idx[i]
        # img_side, seq_len = _get_feat(
        #     opts, h5_data["exps"][temp_idx]["img_side"].value)
        # all_img_side[:seq_len, i, :] = img_side[:seq_len, :]

        # img_front, seq_len = _get_feat(
        #     opts, h5_data["exps"][temp_idx]["img_front"].value)
        # all_img_front[:seq_len, i, :] = img_front[:seq_len, :]
