...
def train_lstm(opts, network, optimizer, criterion, samplers, fid):
    ...
    train_eval = samplers[1]
    test_eval = samplers[2]
    valid_eval = samplers[3]
...
def process_seqs(opts, step, network, sampler, criterion, name):
    ...
    for i in range(sampler.num_batch):
        ...
        sequences_helper.write_predictions2(
            out_dir, exp_names, predict, labels, None, frames, label_names=label_names)

        # process ouputs
        matches = analyze_outs(out_dir, exp_names, predict, labels, label_names)
        # merge the values
...
def analyze_outs(out_dir, exp_names, predict, labels, label_names, frame_thresh=0.7):
    ...
    for i in range(num_labels):
        ...
        temp_label_name = label_names[i]
        if type(temp_label_name) is not str:
            temp_label_name = temp_label_name.decode('UTF-8')
        temp_exp_name = exp_names[0]
        if type(temp_exp_name) is not str:
            temp_exp_name = temp_exp_name.decode('UTF-8')
        output_name = os.path.join(
            out_dir, temp_exp_name, 'processed_%s.csv' % temp_label_name
        )
