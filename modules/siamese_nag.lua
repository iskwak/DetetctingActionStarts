----------------------------------------------------------------------
-- Based on optim.nag... need to clean up later. Uses step learning
-- rate decay rather than inv
--
...
function step_nag.step_nag(opfunc, x, config, state)
    -- (0) get/update state
    local config = config or {}
    local state = state or config
    local lr = config.learningRate or 1e-3
    local lrd = config.learningRateDecay or 0
    local step = config.learningRateDecayStep or 0
    ...
    if state.dfdx then
        ...
        -- unclear if caffe does this. it may just use x as is.
    end
    ...
    -- (2) weight decay
    if wds and wd ~= 0 then
        if not state.weightParameters then
            state.weightParameters = torch.Tensor():typeAs(x):resizeAs(dfdx)
        end
        state.weightParameters:copy(wds):cmul(x)
        dfdx:add(wd, state.weightParameters)
    else
        if wd ~= 0 then
            dfdx:add(wd, x)
        end
    ...
    local clr = lr
    if step ~= 0 then
        local clr = lr * lrd^(math.floor(nevals/step))
    end
    --print(clr)
