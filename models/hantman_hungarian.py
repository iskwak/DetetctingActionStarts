...
gflags.DEFINE_float(
    "hantman_perframe_weight", 0.99, "Perframe weighting")
gflags.DEFINE_float(
...
gflags.DEFINE_string("hantman_arch", "bidirconcat", "Network architecture.")

gflags.DEFINE_string("anneal_type", "exp_step", "line_step, exp_step, or none.")
gflags.DEFINE_float(
    "perframe_decay", 0.99, "decay rate for perframe loss. only used for exp_step")
gflags.DEFINE_float(
    "perframe_decay_step", 1, "decay step rate for perframe loss. only used for exp_step")


gflags.DEFINE_float("perframe_stop", 0.5, "end weight for perframe loss.")
# gflags.DEFINE_float("perframe_start", 1.0, "start weight for perframe loss.")
gflags.DEFINE_float("iter_per_epoch", 1.0, "used internally... don't set.")
...
class HantmanHungarianConcat(nn.Module):
    ...
    def __init__(self, opts=None, input_dims=[64], hidden_dim=64,
                 output_dim=6, label_weight=None, is_bidir=False):
...
class HantmanHungarianBidirProd(nn.Module):
    def __init__(self, opts=None, input_dims=[64], hidden_dim=64,
                 output_dim=6, label_weight=None):
        super(HantmanHungarianBidirProd, self).__init__()
        # if opts is not none, then use opts over provided parameters.
        self.opts = opts
        self.input_dims = input_dims
        self.hidden_dim = hidden_dim
...
class HantmanHungarianBidirConcatSoftmax(nn.Module):
    """Hantman mouse LSTM with hungarian matching."""
    def __init__(self, opts=None, input_dims=[64], hidden_dim=64,
                 output_dim=6, label_weight=None):
        super(HantmanHungarianBidirConcatSoftmax, self).__init__()
        # if opts is not none, then use opts over provided parameters.
        self.opts = opts
        self.input_dims = input_dims
        # https://discuss.pytorch.org/t/trying-to-understand-behavior-of-bidirectional-lstm/4343
...
class HantmanHungarianSoftmax(nn.Module):
    """Hantman mouse LSTM with hungarian matching."""
    def __init__(self, opts=None, input_dims=[64], hidden_dim=64,
                 output_dim=6, label_weight=None, num_layers=2):
        super(HantmanHungarianSoftmax, self).__init__()
        # if opts is not none, then use opts over provided parameters.
        self.opts = opts
        self.input_dims = input_dims
        # https://discuss.pytorch.org/t/trying-to-understand-behavior-of-bidirectional-lstm/4343
        ...
        # with dimensionality hidden_dim.
        self.lstm = nn.LSTM(
            self.hidden_dim, self.hidden_dim, self.n_layers, bidirectional=False
        )
