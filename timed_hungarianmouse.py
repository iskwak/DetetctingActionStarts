...
gflags.DEFINE_string("val_file", None, "Validation data filename (hdf5).")
...
gflags.DEFINE_boolean("threads", False, "Use threads?")
# gflags.DEFINE_float(
...
def _copy_templates(opts, train_data, test_data, val_data):
    ...
    base_out = os.path.join(opts["flags"].out_dir, "predictions", "valid")
    # test_experiments = exp_list[train_vids]
    valid_experiments = val_data["exp_names"].value
    sequences_helper.copy_experiment_graphs(
        opts, base_out, valid_experiments)
...
def _log_outputs(opts, step, network, label_weight, train_data, test_data, val_data):
    ...
    val_cost, val_scores = _process_full_sequences(
        opts, step, network, val_data, "valid", label_weight)

    # apply post processing (hungarian matching and create cleaned outputs).
    # ... this is going to be a problem...
    frame_threshold = [10, 10, 10, 10, 10, 10]
    # ['predict_chew.csv', 'predict_grab.csv', 'predict_hand.csv', 'predict_lift.csv', 'predict_mouth.csv', 'predict_suppinate.csv']
    # frame_threshold = [5, 15, 15, 20, 30, 30]
    # frame_threshold = [30, 15, 15, 5, 30, 20]
    predict_dir = os.path.join(opts["flags"].out_dir,
                               "predictions", "train")
    train_dicts = post_processing.process_outputs(
        predict_dir, "", frame_thresh=frame_threshold)
    ...
    predict_dir = os.path.join(opts["flags"].out_dir,
                               "predictions", "valid")
    val_dicts = post_processing.process_outputs(
        predict_dir, "", frame_thresh=frame_threshold)
    ...
    valf, valf_scores = compute_tpfp(val_dicts)
    ...
    with open(loss_f, "a") as outfile:
        ...
        format_str = ("%d,%f,%f,"
                      ...
                      "%f,%f\n")
        output_data =\
            [step, train_cost, test_cost, trainf, testf, valf] +\
...
def _train_epoch(opts, step, network, optimizer, train_data, test_data, label_weight):
    ...
    if opts["flags"].threads is False:
    ...
    while batch_id != -1:
        ...
        timings[1] += (time.time() - forward_tic)
        ...
        optimizer.step()
        timings[2] += (time.time() - backward_tic)

        step += 1
        batch_num += 1

    g_train_timings[0] += timings[0]
    g_train_timings[1] += timings[1]
    g_train_timings[2] += timings[2]
...
def _train_network(opts, network, optimizer, train_data, test_data, val_data, label_weight):
    """Train the network."""
    # Setup some data stuff
    if opts["flags"].threads is True:
        train_sample_queue = queue.Queue(maxsize=2)
        train_exps = train_data["exp_names"].value

        thread.start_new_thread(
            _thread_mini_batch,
            (opts, train_sample_queue, train_data, train_exps))

        test_sample_queue = queue.Queue(maxsize=2)
        test_exps = test_data["exp_names"].value

        thread.start_new_thread(
            _thread_mini_batch,
            (opts, test_sample_queue, test_data, test_exps))

        val_sample_queue = queue.Queue(maxsize=2)
        val_exps = val_data["exp_names"].value

        thread.start_new_thread(
            _thread_mini_batch,
            (opts, val_sample_queue, val_data, val_exps))

        # create a thread and queue for processing outputs.
        # _predict_write.queue = queue.Queue(maxsize=10)
        # thread.start_new_thread(
        #     _thread_predict_write,
        #     (_predict_write.queue,)
        # )
    ...
    for i in range(opts["flags"].total_epochs):
        ...
        if opts["flags"].threads is False:
            ...
            step = _train_epoch(opts, step, network, optimizer, train_sample_queue, test_data, label_weight)
        print("\tFinished epoch")
        print("\tProcessing all examples...")
        g_train_timings[3] += (time.time() - train_tic)
        network.eval()

        if i % 10 == 0:
            output_tic = time.time()
            if opts["flags"].threads is False:
                _log_outputs(opts, step, network, label_weight, train_data, test_data, val_data)
            else:
                _log_outputs(
                    opts, step, network, label_weight,
                    train_sample_queue, test_sample_queue, val_sample_queue)
            g_output_timings[0] += (time.time() - output_tic)
            g_output_timings[1] += 1
            with open(os.path.join(opts["flags"].out_dir, "output_timing.txt"), "w") as fd:
                fd.write("%f" % g_output_timings[0])
        with open(os.path.join(opts["flags"].out_dir, "train_timing.txt"), "w") as fd:
            fd.write("data: %f\n" % (g_train_timings[0]))
            fd.write("forward: %f\n" % (g_train_timings[1]))
            fd.write("backward: %f\n" % (g_train_timings[2]))
            fd.write("train: %f\n" % (g_train_timings[3]))
...
def _thread_predict_write(data_queue):
    # loop, and wait on the data_queue for what to write
    while 1:
        args = data_queue.get()
        sequences_helper.write_predictions2(*args)
...
def _predict_write(opts, step, out_dir, network, h5_data, exps, label_weight):
    ...
    while batch_id != -1:
        ...
        # for vid in sample_idx:
        for vid_idx in range(len(sample_idx)):
            # vid = sample_idx[vid_idx]
            max_frame = np.argwhere(mask.cpu().data[:, vid_idx, 0].numpy())[-1][0] + 1
            labels.append(temp[:max_frame, vid_idx:vid_idx+1, :])
            frames.append(list(range(max_frame)))
            # import pdb; pdb.set_trace
        ...
        sequences_helper.write_predictions2(
            out_dir, sample_idx, predictions, labels,
            [], frames)
        # else:
        #     # print(sample_idx)
        #     _predict_write.queue.put([
        #         out_dir, sample_idx, predictions, labels, [], frames
        #     ])
...
def _thread_mini_batch(opts, data_queue, h5_data, idx):
    print("starting worker...")
    batch_id = 0
    idx = opts["rng"].permutation(idx)
    while batch_id != -1:
        # print("\tGetting batch: %d" % batch_id)
        inputs, labels, mask, org_labels, sample_idx, batch_id =\
            _get_seq_mini_batch(opts, batch_id, h5_data, idx)
        ...
        # try to queue the data.
        # print("\tAdding data!")
        data_queue.put(
            [inputs, labels, mask, org_labels, sample_idx, batch_id]
        )
        # print("\t%d" % data_queue.qsize())
        # print("\tFinished!")
        # reset the sampler
        if batch_id == -1:
            batch_id = 0
...
def _get_seq_mini_batch(opts, batch_id, h5_data, idx):
    ...
    for i in range(batch_size):
        ...
        seq_len = np.min([h5_data["exps"][temp_idx]["labels"].shape[0],
                          opts["flags"].seq_len])
    ...
    return all_feats, all_labels, all_masks, all_org_labels, idx[vid_idx], batch_id
...
if __name__ == "__main__":
    ...
    # create timings
    g_train_timings = [0, 0, 0, 0]
    g_output_timings = [0, 0]
    # load data
    # try:
    tic = time.time()
    with h5py.File(opts["flags"].train_file, "r") as train_data:
        with h5py.File(opts["flags"].test_file, "r") as test_data:
            with h5py.File(opts["flags"].val_file, "r") as val_data:
                ...
                _train_network(opts, network, optimizer, train_data, test_data, val_data, label_weight)
    # wrap up the output writing
    # if opts["flags"].threads is True:
    #     print("waiting to finish writing to disk")
    #     _predict_write.queue.join()
    #     print("done!")
